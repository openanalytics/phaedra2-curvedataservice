...
public interface CurveRepository extends CrudRepository<Curve, Long> {
  ...
  @Query("select * from curve where wells && array[:wellIds]::bigint[] and result_set_id = (select max(result_set_id) from curve where wells && :wellIds) order by fit_date desc")
